...
def low_memory_state(total_mb):
    ...
    if total_mb < 200:
        return (
            True,
            "nano-critical",
            "Memory is below 200 MB. Avoid full protocol-node deployment; prefer Snell or port forwarding and enable swap.",
        )
    if total_mb < 256:
        return (
            True,
            "nano",
            "Memory is below 256 MB. Treat this as a Nano node; avoid full protocol-node deployment unless swap is available.",
        )
    if total_mb < 512:
        return (
            False,
            "small",
            "Memory is below 512 MB. Full protocol-node deployment may work only with careful swap and low concurrency.",
        )
