...
detect_python() {
  ...
  echo "python3 or python is required for Xray Runtime fixture tests." >&2
  exit 2
}
...
start_fixture() {
  ...
  local args=("${SCRIPT_DIR}/xray-runtime-fixture.py" --host 127.0.0.1 --port 0 --ready-file "$ready_file")
  ...
  if [ ! -f "$ready_file" ]; then
    echo "Xray Runtime fixture did not become ready." >&2
    cat "$log_file" >&2 || true
    exit 1
  fi
...
detect_python
command -v curl >/dev/null 2>&1 || { echo "curl is required for Xray Runtime fixture tests." >&2; exit 2; }
...
XRAY_RUNTIME_E2E_URL="${BASE_URL}" \
  XRAY_RUNTIME_E2E_TOKEN="fixture-token" \
  XRAY_RUNTIME_E2E_WRITE=1 \
  XRAY_RUNTIME_E2E_PORT=12101 \
  bash "${SCRIPT_DIR}/test-xray-runtime-e2e.sh"

echo "Xray Runtime fixture tests passed at ${BASE_URL}"
