...
Required for install:
  ...
  OB_JOIN_TOKEN         One-line join token generated by the master console
...
if [ -z "$MASTER_URL" ]; then
  echo "OB_MASTER_URL is required." >&2
  exit 2
fi

if [ -z "$JOIN_TOKEN" ] && { [ -z "$SERVER_ID" ] || [ -z "$AGENT_TOKEN" ]; }; then
  echo "OB_JOIN_TOKEN is required. Advanced installs may provide OB_SERVER_ID and OB_AGENT_TOKEN instead." >&2
  exit 2
fi
...
register_with_join_token() {
  [ -n "$JOIN_TOKEN" ] || return 0
  if [ -n "$SERVER_ID" ] && [ -n "$AGENT_TOKEN" ]; then
    return 0
  fi

  local master_url host os_name arch manager memory payload response parsed
  master_url="$(normalize_master_url)"
  host="$(hostname 2>/dev/null || uname -n 2>/dev/null || echo controlled-host)"
  os_name="$(detect_os_name)"
  arch="$(uname -m)"
  manager="$(detect_service_manager_hint)"
  memory="$(detect_memory_total_mb)"
  payload="{\"joinToken\":$(json_string "$JOIN_TOKEN"),\"hostname\":$(json_string "$host"),\"host\":$(json_string "$host"),\"agentVersion\":$(json_string "installer"),\"osName\":$(json_string "$os_name"),\"architecture\":$(json_string "$arch"),\"serviceManager\":$(json_string "$manager"),\"memoryTotalMb\":${memory:-0}}"
  echo "Joining Overlord Broil master..."
  response="$(curl -fsS --connect-timeout "$HTTP_CONNECT_TIMEOUT" --max-time "$HTTP_MAX_TIME" -H 'Content-Type: application/json' -d "$payload" "${master_url}/api/v1/agent-join/register")"
  parsed="$(JOIN_RESPONSE="$response" python3 - <<'PY'
import json
import os
import sys
payload = json.loads(os.environ.get("JOIN_RESPONSE") or "{}")
if int(payload.get("code", -1)) != 0:
    raise SystemExit(payload.get("msg") or "agent join failed")
data = payload.get("data") or {}
server_id = data.get("serverId")
agent_token = data.get("agentToken")
if not server_id or not agent_token:
    raise SystemExit("agent join response missing serverId or agentToken")
print(str(server_id))
print(str(agent_token))
PY
)"
  SERVER_ID="$(printf '%s\n' "$parsed" | sed -n '1p')"
  AGENT_TOKEN="$(printf '%s\n' "$parsed" | sed -n '2p')"
  MASTER_URL="$master_url"
  echo "Joined master as server ${SERVER_ID}."
}

register_with_join_token

if [ -z "$SERVER_ID" ] || [ -z "$AGENT_TOKEN" ]; then
  echo "Agent credentials are missing after join." >&2
  exit 2
fi
