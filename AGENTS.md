...
- Snell is unified at the product/node-management layer, but it remains an independent runtime service deployed by the agent, not a native Xray or 3x-ui core protocol.
- Nano controlled hosts below `200 MB` memory should not run full 3x-ui/Xray orchestration through the supported path.
