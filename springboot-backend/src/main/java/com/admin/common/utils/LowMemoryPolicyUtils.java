...
public final class LowMemoryPolicyUtils {
    ...
    public static String advice(Long memoryTotalMb) {
        ...
        if (memoryTotalMb < NANO_CRITICAL_MEMORY_MB) {
            return "Memory is below 200 MB. Avoid full 3x-ui/Xray orchestration; prefer Snell or port forwarding and enable swap.";
        }
        if (memoryTotalMb < NANO_MEMORY_MB) {
            return "Memory is below 256 MB. Treat this as a Nano node; avoid full 3x-ui/Xray unless swap is available.";
        }
        if (memoryTotalMb < SMALL_MEMORY_MB) {
            return "Memory is below 512 MB. Full 3x-ui/Xray may work only with careful swap and low concurrency.";
        }
        return null;
    }
}
