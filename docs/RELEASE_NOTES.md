...
- Future branch update: agent maintenance now also covers delayed uninstall, install diagnostics, ACME/certificate diagnostics, firewall diagnostics, one-click Xray Panel/Xray/Snell repair and failed-task retry from the task card.
...
- Future branch update: added Nano controlled-server detection from agent heartbeat memory totals. The master stores `nano-critical` below `200 MB`, `nano` below `256 MB`, `small` below `512 MB`, raises low-memory alerts and blocks full Xray Panel/Xray orchestration plus Xray protocol-node creation on `nano-critical` hosts.
- Future branch update: added the Runtime Provider registry for `xrayPanel`, `snell`, `forward`, `certificate` and `firewall`; deployment tasks now expose provider metadata to the master UI and controlled-agent claim payloads.
...
- Future branch update: added the State Sync runtime overview API and control-center panel. The master aggregates latest task runtime states with server heartbeat service/certificate fields, producing a server-by-provider view for Xray Panel/Xray, Snell and certificate health.
- Future branch update: State Sync rows can now create provider-aware `agent-maintenance` diagnostics and Xray Panel/Snell repair tasks, keeping the visible state panel connected to the same controlled-agent execution/report path.
- Future branch update: `agent-maintenance logs` now returns structured `logs.items` for Overlord agent, Xray Panel/Xray, Snell, forwarding and task-log sources, and the master task card can show a remote-log summary before raw output.
...
- Future branch update: added `scripts/test-xray-panel-e2e.sh` plus a manual `Real Xray Panel E2E` GitHub workflow. The gate checks real Xray Panel status, inbound list and Xray config when endpoint/token secrets exist, and can explicitly create/toggle/delete a temporary inbound when write mode is enabled.
- Future branch update: Xray Panel deployment tasks now produce agent-executable scripts that call the Xray Panel inbound add/delete/restart APIs and report inbound metadata back to the master.
- Future branch update: agent task results now redact Xray Panel API tokens, passwords, 2FA codes and `serverSecrets` before storing task history, while still allowing encrypted server metadata updates.
...
- Future branch update: real Xray Panel E2E now accepts newer Xray Panel status responses with `obj: null`; `isrco-hk` passed direct `Xray Panel 3.1.0` inbound add/toggle/delete and Overlord master API add/toggle/delete against a temporary Xray Panel container.
...
| Runtime Provider / Runtime State audit | Task claim/report results carry provider metadata and normalized runtime state for `xrayPanel`, `snell`, `forward`, `certificate` and `firewall`, giving the master a stable audit trail across agent versions. |
...
| State Sync actions | Runtime rows can generate provider-aware diagnostics and Xray Panel/Snell repairs as normal `agent-maintenance` tasks for the controlled agent. |
...
| Remote runtime logs | `agent-maintenance logs` reports structured `logs.items` for Overlord agent, Xray Panel/Xray, Snell, forwarding and task logs, with task-card summaries in the master UI. |
...
| Xray Panel | API fixture remains in CI, and `isrco-hk` has a recorded real `Xray Panel 3.1.0` container write smoke for direct API plus Overlord master API inbound add/toggle/delete; full install/configure still targets systemd hosts. |
| Snell | Product-level protocol node with separate systemd/OpenRC runtime, not a native Xray/Xray Panel core protocol; live smoke script is available for authorized hosts. |
| Verification | Shell syntax, agent mock, SQLite schema smoke, Xray Panel fixture, optional real Xray Panel E2E gate, master port contract, frontend build, backend Maven build, install matrix and MySQL/SQLite single-image compose smoke. |
...
- The included Xray Panel fixture is API-level. One real `isrco-hk` container write smoke is now recorded, but a `1.0` claim still needs a broader VPS/provider matrix with endpoint secrets configured.
- Snell is unified at the product/control-plane layer. It remains a separate runtime service managed by the Overlord agent rather than a native Xray protocol inside Xray Panel.
- Nano detection is a protection layer, not a promise that Xray Panel/Xray will run well on tiny hardware. Sub-200 MB hosts should be treated as Snell or forwarding nodes unless swap and real-host testing prove otherwise.
...
- Added `scripts/release-check.sh` as the single pre-release gate for shell syntax, agent mock execution, tokenized Xray Panel fixture coverage, compose validation, frontend build, Docker Maven backend build, disposable compose smoke and `git diff --check`.
...
- Kept credential hardening in the release baseline: stored agent tokens and Xray Panel API/password/2FA fields are encrypted through `SECRET_ENCRYPTION_KEY`, with legacy plaintext rows still readable for gradual upgrades.
...
| Xray Panel | API-token inbound/client flows, Xray config/outbound reads, traffic sync and Xray restart; full install/configure orchestration requires systemd. |
...
| Full Xray Panel install/configure | Supported | Supported | Not supported in `0.5.0`; use a systemd host |

### Known Post-Release Enhancements

- Add real disposable Xray Panel container orchestration smoke tests once a stable upstream-compatible fixture is selected.
- Add a documented key-rotation migration for encrypted Xray Panel credentials and API tokens.
...
- Sensitive stored agent tokens and Xray Panel API/password/2FA fields are encrypted through `SECRET_ENCRYPTION_KEY`, with plaintext legacy rows still readable.
- The Xray Panel fixture now covers Bearer token success, missing-token and wrong-token paths in addition to inbound/outbound/config/traffic/restart routes.
...
- Multi-server orchestration workspace for registered servers, Xray Panel connection settings, protocol nodes, Snell nodes, port forwards, deployment tasks and traffic snapshots.
- Xray Panel connector coverage for connection tests, inbound CRUD, client operations, full Xray config reads, outbound traffic reads, Xray restarts and traffic sync.
- One-click orchestration tasks that can install or reuse Xray Panel, create starter Xray protocols, install Snell and return runtime metadata.
- Unified rule center and monitor alert center in the master workspace for day-2 operations across servers.
- Stateful Xray Panel API fixture for regression coverage of inbound/client/outbound/config/traffic/restart routes.
...
| Xray Panel management | API-token flows for inbounds and clients; login+CSRF path for full Xray/outbound operations. |
...
| Traffic | Manual and scheduled Xray Panel traffic sync into local snapshots. |
...
| Xray Panel regression | Local Python fixture and shell test covering tokenized core Xray Panel proxy routes. |
| CI verification | Backend/frontend build, agent mock test, Xray Panel fixture test, compose dry run and disposable compose smoke test. |
...
docker run --rm -v "$PWD:/workspace" -v ob_xray-panel_frontend_node_modules:/workspace/vite-frontend/node_modules -w /workspace/vite-frontend node:22-bookworm bash -lc "npm install --legacy-peer-deps --no-audit --no-fund && npm run build"
```
...
- The included Xray Panel fixture is API-level; real-container Xray Panel orchestration smoke tests should still be added before a broad 1.0 compatibility claim.
