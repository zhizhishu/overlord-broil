...
当前控制台默认就是中文 UI。主控中心、服务器卡片、一键编排、3x-ui 入站/出站、Snell 节点、远端转发、Agent 诊断/日志/升级等核心操作入口都以中文为主。

现在没有做完整中英双语 i18n 切换器，因为项目主要面向中文使用场景；为了避免“按钮切了语言但页面还有一半没翻译”的假切换，现阶段先把中文作为默认产品语言。后续如果要同时面向英文用户，应新增正式 i18n 字典，再提供中文/英文切换。
