...
| 页面 | 截图 |
| --- | --- |
| 登录 | [docs/assets/live-login.png](docs/assets/live-login.png) |
| 仪表盘 | [docs/assets/live-control-overview.png](docs/assets/live-control-overview.png) |
| 服务器 | [docs/assets/live-servers.png](docs/assets/live-servers.png) |
| 入站节点 | [docs/assets/live-inbounds.png](docs/assets/live-inbounds.png) |
| 出站与路由 | [docs/assets/live-routes.png](docs/assets/live-routes.png) |
| 转发/隧道 | [docs/assets/live-tunnels.png](docs/assets/live-tunnels.png) |
| 流量 | [docs/assets/live-traffic.png](docs/assets/live-traffic.png) |
| 证书 | [docs/assets/live-certificates.png](docs/assets/live-certificates.png) |
| 设置 | [docs/assets/live-settings.png](docs/assets/live-settings.png) |
...
被控服务器只需要开放你创建的业务端口, 例如可选节点服务端口 `5168`, Xray inbound 端口, Snell 端口, 远端转发端口, 以及 ACME HTTP 模式需要的 `80/tcp`.
...
先在主控里创建服务器, 然后从服务器卡片点击 `加入命令`. 主控会生成一条只包含 `OB_MASTER_URL` 和一次性 `OB_JOIN_TOKEN` 的安装命令, token 24 小时后过期, 被控会自动换取内部 serverId 和 agent token. 在被控服务器执行:
...
- 被控命令行: `curl -fsSL https://raw.githubusercontent.com/zhizhishu/overlord-broil/main/scripts/install-agent.sh | sudo bash -s -- uninstall --yes`, 会移除本机服务、脚本、凭据和 `/var/lib/overlord-agent` 工作目录.
- 被控 UI: 在服务器卡片点击 `卸载 Agent`, 二次确认后主控会生成 `uninstall-agent` 任务; 被控回报结果后会延迟移除服务、脚本和本机凭据.
- 记录清理: 确认被控离线后再点 `删除记录`, 这个动作只删除主控侧记录, 不会替你卸载远端 Agent.
...
| 内存 | 策略 |
| --- | --- |
| `< 200 MB` | Nano critical. 阻止完整节点服务部署和重型协议节点创建, 建议 Snell 或远端转发 |
| `< 256 MB` | Nano. 显示强提醒, 只建议轻量任务 |
| `< 512 MB` | Small. 需要 swap 和低并发才适合跑完整代理栈 |

Alpine/OpenRC 支持 Agent, Snell 和远端转发. 完整节点服务安装 / 配置主要面向 systemd 主机.
...
真实节点服务合同烟测是可选项. 没有配置真实地址和 API token 时会自动跳过:
