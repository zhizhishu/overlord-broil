...
Overlord Broil 是一个独立的主控 / 被控运维面板, 用来统一管理多服务器上的 Xray / Reality, Snell, 端口转发, 证书, 防火墙, 流量同步和 Agent 任务.
...
| Provider | 管理内容 |
|     |     |
| xrayPanel | Xray, Reality, inbound/outbound, 路由规则, IPv4/IPv6 策略, 流量同步, Xray restart |
...
Snell 已统一到产品层的“协议节点”管理里, 但它不是 Xray 的原生协议. 底层仍由 Agent 在被控服务器上部署独立 Snell 服务, 这样更符合 Snell 的真实运行方式.
...
被控服务器只需要根据你创建的业务节点开放实际协议端口, 例如兼容面板端口建议 `5168`, Xray inbound 端口, Snell 端口和远端转发端口.
...
5. 多选服务器, 执行一键编排: 安装或复用兼容面板运行时, 创建 Xray/Reality 节点, 部署 Snell, 配置转发, 同步状态.
...
| 档位 | 内存 | 策略 |
|     |     |     |
| nano critical | `< 200 MB` | 阻止完整 Xray 编排, 建议只跑 Snell 或端口转发 |
| nano | `< 256 MB` | 显示 Nano 提醒, 谨慎创建重运行时任务 |
| small | `< 512 MB` | 显示小内存提醒 |
| standard | `>= 512 MB` | 正常路径 |

这只是保护层, 不保证 Xray 能在超小机器长期稳定运行.
...
| 完整 Xray 面板安装配置 | 支持 | 支持 | `0.6.0` 暂不支持 |
...
真实 Xray Panel 合同烟测是可选项; 没有配置真实地址和 API token 时会自动跳过.

真实验证记录: `isrco hk` 上的 `authorized disposable Xray Panel runtime image` (`Xray Panel 3.1.0`) 已通过直连 Xray Panel E2E 写入合同, 也通过 Overlord 主控 API 的 inbound 新增 / 启停 / 删除链路. 临时端口 `42123` 和 `42124` 已清理.
...
## 仍未到 1.0 的部分
  ...
  还需要把当前 `isrco hk` 真实兼容面板容器验证扩展到更多 VPS / 云厂商目标.
...
本项目是独立项目, 不是下列项目的官方发行版, 但参考并感谢它们:
  ...
  Xray compatible panel APIs: inbound, outbound, routing, traffic and runtime-management behavior.
...
只在你拥有授权的服务器上使用本项目. 请修改默认密码, 保管好 `/opt/overlord broil/.env`, 不要把 `SECRET_ENCRYPTION_KEY`, Agent token, 面板 API token 或 Cookie 公开到日志, Issue, 截图或聊天记录里.